...
from v1.utils.driver_manager import DriverManager
...
async def abort(request: SMUBaseRequest):
    ...
    smu = driver_manager.get_instance(request.instrument_name, request.instrument_model_name, request.instrument_address)
...
async def close(request: SMUBaseRequest):
    ...
    driver_manager.remove_instance(request.instrument_name, request.instrument_model_name, request.instrument_address)
...
async def connect_disconnect(request: ConnectDisconnectRequest):
    ...
    smu = driver_manager.get_instance(request.instrument_name, request.instrument_model_name, request.instrument_address)
    print(f"Connecting to {request.instrument_name} at {request.instrument_address} on channel {request.channel}")
